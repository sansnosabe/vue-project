      ...
      <v-col v-for="(item, index) in items" :key="index" cols="12" sm="6" md="4">
        <v-card class="mx-auto" max-width="344" outlined>
          <v-list-item three-line>
            ...
            <v-list-item-avatar tile size="80" color="grey" class="rounded">
              <img :src="item.avatar" alt="Avatar" />
            </v-list-item-avatar>
            <!-- Otra manera de pasar avatares random a los users -->
            <!-- <v-list-item-avatar tile size="80" color="grey" class="rounded">
              <img :src="getAvatarUrl(item.id)" alt="Avatar" />
            </v-list-item-avatar> -->
          </v-list-item>
...
export default {
  ...
  methods: {
    ...
    // Otra manera de pasar un avatar para los usuarios
    // getAvatarUrl(userId) {
    //   return `https://avatars.dicebear.com/api/avataaars/${userId}.svg`;
    // },
  },
};
</script>
